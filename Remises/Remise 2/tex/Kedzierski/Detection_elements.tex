...
Cette méthode est beaucoup plus fiable que celle utilisée au départ. Notre méthode initiale était de détecter le nombre de sommets des formes détectées pour ensuite déduire quelle forme il s'agissait. Cette méthode avait une faiblesse particulièrement pour les formes qui se ressemblent, comme le cercle et le pentagone. Cependant, avec notre nouvelle méthode, les taux de compatibilités sont très précis. En observant la figure \ref{f:PrecisionDetection}, une île ronde possède un taux de compatibilité d'environ 100 fois plus élevé à un cercle qu'à un pentagone.
